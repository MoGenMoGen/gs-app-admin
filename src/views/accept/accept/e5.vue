...
<!--            <van-field v-model="info.estateNm" label="小区名称：" readonly  label-width="80" >  </van-field>-->
            ...
            <van-field name="switch" label="无负压" label-width="120">
                <slot slot="input">
                    <van-switch v-model="info.wfy" size="20" :active-value="1" :inactive-value="0" active-color="#07c160" inactive-color="#ee0a24"></van-switch>
                </slot>
            </van-field>
            ...
            <van-field name="switch" label="箱式" label-width="120">
                <slot slot="input">
                    <van-switch v-model="info.xs" size="20" :active-value="1" :inactive-value="0" active-color="#07c160" inactive-color="#ee0a24"></van-switch>
                </slot>
            </van-field>
            ...
            <van-field name="switch" label="水箱加变频" label-width="120">
                <slot slot="input">
                    <van-switch v-model="info.sxbp" size="20" :active-value="1" :inactive-value="0" active-color="#07c160" inactive-color="#ee0a24"></van-switch>
                </slot>
            </van-field>
            ...
            <van-field name="switch" label="工频" label-width="120">
                <slot slot="input">
                    <van-switch v-model="info.gp" size="20" :active-value="1" :inactive-value="0" active-color="#07c160" inactive-color="#ee0a24"></van-switch>
                </slot>
            </van-field>
...
<!--            <van-field v-model="info.deviceNum" label="设备套数："  placeholder="请输入套数"  label-width="80">  </van-field>-->
            ...
            <van-field name="radio" label="验收结论：" label-width="80" @click="off(info.val2,'每台水泵的进出水管管径至少比水泵口管径大一个规格，每台水泵进出口处安装检修阀，进出管道应加设柔性接头，在水泵出口处和检修阀之间装设橡胶瓣止回阀，检修阀采用蝶阀或软密封闸阀。')">
                <slot slot="input">
            ...
            <van-field name="radio" label="验收结论：" label-width="80" @click="off(info.val4,'不锈钢钢板、封头、管道及配件、螺栓（丝）、螺帽、垫片、管道、法兰等材质应采用不低于SUS304的食品级不锈钢材质。')">
                <slot slot="input">
            ...
            <van-field name="radio" label="验收结论：" label-width="80" @click="off(info.val6,'泵房设备安装应考虑减震防噪措施，水泵基座与基础之间、水泵与水泵基座之间都应进行隔振处理，隔振原件可采用橡胶隔振垫或弹簧隔振器，进出水管穿越地下室或地下构筑物壁板时应设置防水和隔振措施，管道支架应采用隔振支架，隔振支架与管道之间应有隔振件')">
                <slot slot="input">
            ...
            <van-field name="radio" label="验收结论：" label-width="80" @click="off(info.val7,'水泵的出水管上应设压力表、阀门和橡胶瓣止回阀（符合多功能阀安装条件的出水管，可用多功能阀取代止回阀），应设置防水锤装置，安装位置在每个分区的出水总管处。')">
                <slot slot="input">
            ...
            <van-field name="radio" label="验收结论：" label-width="80" @click="off(info.val10,'泵房内进出水管均采用不锈钢管，连接方式为DN100（含）以上沟槽方式，DN100以下双卡压方式，不锈钢管设计安装终点为距离泵房外墙1.5m处，明管应采用不少于30mm厚度的橡塑保温棉(不锈钢管除外)，埋地管采用钢管套管')">
                <slot slot="input">
            ...
            <van-field name="radio" label="验收结论：" label-width="80" @click="off(info.val12,'泵房内管道应进行标识：进水总管及水箱出水管－天酞蓝色；出水管：低区—绿色，中区—白色，高区—深蓝色，超高区—黄色。排水管、溢流管－黑色，表示水流方向的箭头与对应管道文字同色')">
                <slot slot="input">
            ...
            <van-cell>
                <span class="span">
                   13.泵房管道材质采用不低于SUS304的食品级不锈钢材质，
                    <br>&nbsp;&nbsp;&nbsp;口径DN50,厚度（<input class="input1" v-model="info.val69" type="number">    ）；
                    <br>&nbsp;&nbsp;&nbsp;口径DN80，厚度（ <input class="input1" v-model="info.val70" type="number">   ）；
                    <br>&nbsp;&nbsp;&nbsp;口径DN100，厚度（ <input class="input1" v-model="info.val71" type="number">   ）；
                    <br>&nbsp;&nbsp;&nbsp;口径DN150，厚度（ <input class="input1" v-model="info.val72" type="number">   ）；
                    <br>&nbsp;&nbsp;&nbsp;口径DN200，厚度（ <input class="input1" v-model="info.val73" type="number">   ）；
                   <br>&nbsp;&nbsp;&nbsp;口径300，厚度（ <input class="input1" v-model="info.val74" type="number">   ）；
                </span>
            </van-cell>
            <van-field name="radio" label="验收结论：" label-width="80" @click="off(info.val12,'泵房内管道应进行标识：进水总管及水箱出水管－天酞蓝色；出水管：低区—绿色，中区—白色，高区—深蓝色，超高区—黄色。排水管、溢流管－黑色，表示水流方向的箭头与对应管道文字同色')">
                <slot slot="input">
            ...
            <van-field name="radio" label="验收结论：" label-width="80" @click="off(info.val13,'无负压供水设备进水管上必须设置低阻力倒流防止器，应水平安装，倒流防止器排水口应通过管道接至排水沟')">
                <slot slot="input">
            ...
            <van-field name="radio" label="验收结论：" label-width="80" @click="off(info.val15,'设置水箱的进水管宜与加压泵组进水管之间安装旁通管，满足临时直抽功能')">
                <slot slot="input">
            ...
            <van-field name="radio" label="验收结论：" label-width="80" @click="off(info.val16,'在水箱总出水管与水泵之间合适位置，安装一个DN20水质取样龙头。')">
                <slot slot="input">
            ...
            <van-field name="radio" label="验收结论：" label-width="80" @click="off(info.val57,'泵房大门安装“泵房重地，闲人免进”的警示牌及“**小区泵房”，格式和材质符合要求。')">
                <slot slot="input">
            ...
            <van-field name="radio" label="验收结论：" label-width="80" @click="off(info.val58,'泵房制度牌按要求配备并上墙、泵房其他标识标牌符合验收要求（无负压泵房2块，有水箱泵房3块，标化泵房按约定配置）。')">
                <slot slot="input">
            ...
            <van-field  v-model="info.val60"  rows="3"   autosize    label="其他补充：" type="textarea"  placeholder="请输入" label-width="80" ></van-field>
