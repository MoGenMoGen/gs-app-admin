  ...
  <div id="container">
    ...
    >
      <div v-for="(item, index) in dataList" :key="item.id" class="listItem">
        ...
        <div
          style="margin-left: 10px; padding-bottom: 10px"
          @click="getSpecInfo(item.id)"
          v-if="tabId != 0 && tabId != 1"
        >
          <van-tag type="success" v-if="item.confinedStatus == '2'"
            >受限空间审核通过</van-tag
          >
          <van-tag type="primary" v-if="item.confinedStatus == '1'"
            >受限空间审核中</van-tag
          >
          <van-tag type="warning" v-if="item.confinedStatus == '0'"
            >受限空间未提交</van-tag
          >
          <!--          <span style="font-size: 10px">清洗单位: {{ item.unitNm }}</span>-->
        </div>
