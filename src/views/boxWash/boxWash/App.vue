    ...
    <div id="container">
                    ...
                    <div v-for="item in dataList" :key="item.id" @click="toDetail(item)">
                        <van-cell-group>
                            ...
                            <van-field label="泵房名称:" v-model="item.pumpNm" readonly :border="false"
                                       label-width="120"></van-field>
                            <van-field label="行政区域:" v-model="item.area" readonly :border="false" label-width="120"
                                       right-icon="arrow"></van-field>
                            <van-field label="水箱容积（吨）:" v-model="item.volume" readonly :border="false"
                                       label-width="120"></van-field>
                            <van-field label="清洗消毒时间:" v-model="item.disinfectTm" readonly :border="false"
                                       label-width="120"></van-field>
        ...
        <van-popup v-model="show" position="bottom" :style="{ height: '90%' }" closeable round close-icon="close">
            ...
            <van-cell-group style="margin-top: 50px">
                ...
                <van-field v-if="info.cleanRecord !== null " v-model="info.cleanRecord" label="清洗消毒记录：" label-width="120" readonly></van-field>
                <van-field v-model="info.recorder" label="记录人：" label-width="120" readonly></van-field>
                <van-field v-model="info.leader" label="负责人：" label-width="120" readonly></van-field>
    ...
    export default {
        ...
        methods: {
            ...
            toDetail(item) {
                ...
                if (this.info.val8 === 1){
                    if (item.val4){
                        this.val4 = item.val4.split(",");
                    }
                    if (item.val3){
                        this.val3 = item.val3.split(",");
                    }
                    this.from.val5 = Number(item.val5);
                    this.from.val6 = item.val6;
                    this.from.val7 = item.val7;
                    this.from.val8 = item.val8;
                }
