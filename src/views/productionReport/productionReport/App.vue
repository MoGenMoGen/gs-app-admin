  ...
  <div id="container">
    ...
    <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad" :immediate-check="immediate">
      <div v-for="(item,index) in dataList" :key="index" @click="toDetail(item)">
        <van-cell-group>
          ...
          <van-field label="进水压力:" v-model="item.inPressure" readonly :border="false" label-width="120"
    ...
    <van-popup v-model="show" position="bottom" :style="{ height: '80%' }" closeable round close-icon="close">
      <van-cell-group style="margin-top: 50px">
        <van-field label="1#频率（HZ）:" v-model="info.operatingFrequency1" readonly :border="false"
                   label-width="120"></van-field>
        <van-field label="2#频率（HZ）:" v-model="info.operatingFrequency2" readonly :border="false"
                   label-width="120"></van-field>
        <van-field label="3#频率（HZ）:" v-model="info.operatingFrequency3" readonly :border="false"
                   label-width="120"></van-field>
        <van-field label="4#频率（HZ）:" v-model="info.operatingFrequency4" readonly :border="false"
                   label-width="120"></van-field>
        ...
        <van-field v-if="info.pump1 === 0" label="1#泵状态:" value="停止" readonly label-width="120"
                   :border="false"></van-field>
        <van-field v-if="info.pump1 === 1" label="1#泵状态:" value="运行" readonly label-width="120"
                   :border="false"></van-field>
        <van-field v-if="info.pump1 === 2" label="1#泵状态:" value="故障" readonly label-width="120"
                   :border="false"></van-field>

        <van-field v-if="info.pump2 === 0" label="2#泵状态:" value="停止" readonly label-width="120"
                   :border="false"></van-field>
        <van-field v-if="info.pump2 === 1" label="2#泵状态:" value="运行" readonly label-width="120"
                   :border="false"></van-field>
        <van-field v-if="info.pump2 === 2" label="2#泵状态:" value="故障" readonly label-width="120"
                   :border="false"></van-field>

        <van-field v-if="info.pump3 === 0" label="3#泵状态:" value="停止" readonly label-width="120"
                   :border="false"></van-field>
        <van-field v-if="info.pump3 === 1" label="3#泵状态:" value="运行" readonly label-width="120"
                   :border="false"></van-field>
        <van-field v-if="info.pump3 === 2" label="3#泵状态:" value="故障" readonly label-width="120"
                   :border="false"></van-field>

        <van-field v-if="info.pump4 === 0" label="4#泵状态:" value="停止" readonly label-width="120"
                   :border="false"></van-field>
        <van-field v-if="info.pump4 === 1" label="4#泵状态:" value="运行" readonly label-width="120"
                   :border="false"></van-field>
        <van-field v-if="info.pump4 === 2" label="4#泵状态:" value="故障" readonly label-width="120"
                   :border="false"></van-field>


        <van-field v-if="info.valveStatus === 0" label="阀门状态:" value="开到位" readonly label-width="120"
                   :border="false"></van-field>
        <van-field v-if="info.valveStatus === 1" label="阀门状态:" value="关到位" readonly label-width="120"
                   :border="false"></van-field>
        <van-field v-if="info.valveStatus === 2" label="阀门状态:" value="故障" readonly label-width="120"
                   :border="false"></van-field>
    ...
    <van-popup v-model="searchShow" position="bottom" :style="{ height: '80%' }" closeable round close-icon="close">
      <van-form @submit="search" style="margin-top: 50px">
        ...
        <van-field :value="searchData.time" label="日期" readonly clickable name="date" placeholder="点击选择日期"
                   @click="showPicker = true" label-width="140"></van-field>
        <div style="height: 20px"></div>
