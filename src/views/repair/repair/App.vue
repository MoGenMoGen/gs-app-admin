...
<!--                            <van-field label="申请单位:" v-model="item.applicantUnitNm" readonly :border="false" label-width="120"></van-field>-->
...
<template>
  <div id="container">
      ...
      <van-tab v-for="item in tabList" :title='item' :key="item">
        <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad"
          ...
          <div v-for="item in dataList" :key="item.id" @click="toDetail(item)">
            <van-cell-group>
              <van-field label="维修单号:" v-model="item.no" readonly :border="false" label-width="120"></van-field>
              <van-field label="部门:" v-model="item.org" readonly :border="false" label-width="120"></van-field>
              <van-field label="泵房名称:" v-model="item.pumpNm" readonly :border="false" label-width="120"></van-field>
              <!--                            <van-field label="申请单位:" v-model="item.applicantUnitNm" readonly :border="false" label-width="120"></van-field>-->
              <van-field label="维修单位:" v-model="item.repairUnitNm" readonly :border="false"
                         label-width="120"></van-field>
              <van-field label="申请日期:" v-model="item.applicantTm" readonly :border="false"
                         label-width="120"></van-field>
              <van-field label="预算:" v-model="item.budget" readonly :border="false" label-width="120"></van-field>
              <van-field label="维修事由:" v-model="item.cause" readonly :border="false" label-width="120"></van-field>
              <div style="margin-left: 10px;margin-bottom: 5px">
    ...
    <van-popup v-model="show" position="bottom" :style="{ height: '95%' }" closeable round close-icon="close">
      ...
      <van-field v-if="info.opinion1Status !== 0" :readonly="info.updBy !== '1'" v-model="info.opinion1" rows="2"
                 autosize label-width="140"
                 label="经办人意见:" type="textarea" placeholder="请输入"></van-field>
      <van-field v-if="info.opinion2Status !== 0" :readonly="info.updBy !== '2'" v-model="info.opinion2" rows="2"
                 autosize label-width="140"
                 label="设施科副科长意见:" type="textarea" placeholder="请输入"></van-field>
      <van-field v-if="info.opinion3Status !== 0" :readonly="info.updBy !== '3'" v-model="info.opinion3" rows="2"
                 autosize label-width="140"
                 label="设施科科长意见:" type="textarea" placeholder="请输入"></van-field>
      <van-field v-if="info.opinion4Status !== 0" :readonly="info.updBy !== '4'" v-model="info.opinion4" rows="2"
                 autosize label-width="140"
                 label="分管经理意见:" type="textarea" placeholder="请输入"></van-field>
      <van-field v-if="info.opinion5Status !== 0" :readonly="info.updBy !== '5'" v-model="info.opinion5" rows="2"
                 autosize label-width="140" label="经理意见:"
                 type="textarea" placeholder="请输入"></van-field>
  ...
  methods: {
    ...
    upd(val) {
      Dialog.confirm({
        ...
        let nickname = JSON.parse( this.until.loGet("userInfo")).nickname;
        let format = this.dateFormat("YYYY-mm-dd HH:MM:SS", new Date())
        if (this.info.updBy === "1") {
          if (this.info.opinion1 == '') {
            return Toast.fail('请输入意见')
          }
        ...
        if (this.info.updBy === "3") {
          ...
          this.info.opinion3Status = val;
          this.info.opinion3Tm = format
          this.info.opinion3Nm = nickname
        ...
        if (this.info.updBy === "4") {
          ...
          this.info.opinion4Status = val;
          this.info.opinion4Tm = format
          this.info.opinion4Nm = nickname
        }
        ...
        if (this.info.updBy === "5") {
          ...
          this.info.opinion5Status = val;
          this.info.opinion5Tm = format
          this.info.opinion5Nm = nickname
        }
        this.api.updRepair(JSON.stringify(this.info)).then(res => {
          this.show = false;
          Toast('审批完成');
...
>>>>>>> ee3cf6319d4f31c7204c1e948021dc483640c04d
